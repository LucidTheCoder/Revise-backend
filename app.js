...
function renderDisplaySettings(el) {
  ...
  const fsBtns = ["small", "normal", "large", "xlarge"]
    .map(
      (v) => `
  ...
  const dsBtns = ["compact", "normal", "large"]
    .map(
      (v) => `
...
async function handleRegister() {
  ...
  const emailConfirm = document
    .getElementById("register-email-confirm")
    .value.trim();
  ...
  if (email !== emailConfirm) {
    errEl.textContent = "Email addresses do not match.";
    document.getElementById("register-email-confirm").focus();
    return;
  }
  ...
  try {
    ...
    syncServerStats(data.user);
    showSignInBanner(
      "Account created! Welcome, " + data.user.name.split(" ")[0] + ".",
    );
  } catch {
    errEl.textContent = "Network error.";
  } finally {
...
async function signInWithDiscord() {
  ...
  if (!clientId) {
    showToast(
      "Discord login is not configured yet. Add DISCORD_CLIENT_ID to your environment.",
    );
    return;
  }
...
async function signInWithGoogle() {
  ...
  const client = google.accounts.oauth2.initTokenClient({
    client_id:
      "967159024316-cj20g0uo2ekvclrrglepjqtug933d20f.apps.googleusercontent.com",
...
function scientificSymbols(input) {
  ...
  text = text.replace(
    /([A-Za-z0-9()]+)\^([0-9]*)([+-])/g,
    (_, base, mag, sign) => {
      const formattedBase = String(base).replace(
        /([A-Za-z\)])([0-9n]+)/g,
        (m, a, b) => `${a}${toSubscript(b)}`,
      );
...
function colorVar(subjectId) {
  const custom = normalizeHexColor(
    state.subjectMap.get(subjectId)?.primaryColor,
  );
...
function subjectReleaseInfo(subject) {
  ...
  const releaseDateObj = releaseDateRaw
    ? new Date(`${releaseDateRaw}T00:00:00`)
    : null;
  const hasFutureDate = !!(
    releaseDateObj &&
  ...
  const releaseLabel = hasFutureDate
    ...
    : "";
  const badge =
    String(subject?.comingSoonBadge || "").trim() ||
    (hasFutureDate ? `Coming ${releaseLabel}` : isWip ? "WIP" : "");
...
function buildTopicDiagramSvg(topic) {
  ...
  const PAL = {
    ...
    phy: {
      ...
      li: "rgba(129,140,248,.55)",
    },
  };
  let P = PAL[sub] || PAL.chem;
  const customPrimary = normalizeHexColor(
    state.subjectMap.get(topic.subject)?.primaryColor,
  );
  if (customPrimary) {
    P = {
      ...P,
  ...
  const H6 = (cx, cy, r, fi, st) =>
    `<circle cx="${cx}" cy="${cy}" r="${(r * 1.18).toFixed(1)}" fill="#161b22"/><polygon points="${hexPts(cx, cy, r)}" fill="${fi}" stroke="${st}" stroke-width="2" stroke-linejoin="round"/>`;
  const HT = (cx, cy, lines, sz = 11) => {
    ...
    return lines
      .map(
        (l, i) =>
          `<text x="${cx}" y="${(y0 + i * lh).toFixed(1)}" text-anchor="middle" dominant-baseline="auto" fill="#e6edf3" font-size="${sz}" font-family="DM Sans" font-weight="500">${escapeXml(String(l || ""))}</text>`,
      )
      .join("");
  };
  const TT = (t) =>
    `<text x="22" y="30" fill="${P.ac}" font-size="12.5" font-family="DM Sans" font-weight="700" letter-spacing=".3">${escapeXml(t)}</text>`;
  const LN = (x1, y1, x2, y2, col, w = 1.8, dash = "") =>
    `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${col || P.li}" stroke-width="${w}" stroke-linecap="round"${dash ? ` stroke-dasharray="${dash}"` : ""}/>`;
  const AR = (x1, y1, x2, y2, col) =>
    `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${col || P.li}" stroke-width="1.8" stroke-linecap="round" marker-end="url(#ah${tid})"/>`;
  const LB = (x, y, t, col, sz = 9.5, a = "middle") =>
    `<text x="${x}" y="${y}" text-anchor="${a}" fill="${col || "#8b949e"}" font-size="${sz}" font-family="DM Sans">${escapeXml(String(t || ""))}</text>`;
  const CRV = (d, col, w = 2) =>
    `<path d="${d}" fill="none" stroke="${col || P.li}" stroke-width="${w}" stroke-linecap="round"/>`;
  ...
  const spokeMap = (
    ...
    const lines = sp
      .slice(1)
      .map((s) => AR(sp[0].cx, sp[0].cy, s.cx, s.cy, P.li))
      .join("");
    // Draw hexes on top
    const hub =
      H6(sp[0].cx, sp[0].cy, hubR, hfi, P.st) +
      HT(
        sp[0].cx,
        sp[0].cy,
        Array.isArray(hubLabel) ? hubLabel : [hubLabel],
        hubR > 36 ? 13 : 11,
      );
    const sats = sp
      .slice(1)
      .map((s, i) => {
        ...
        return (
          H6(s.cx, s.cy, satR, sfi, P.st) +
          HT(s.cx, s.cy, Array.isArray(lbl) ? lbl : [lbl], 9)
        );
      })
      .join("");
  ...
  if (sub === "chem") {
    ...
    if (tid.includes("energetics")) {
      const BY = 205,
        EXH = 90,
        ENH = 60; // baseline y, exo height, endo height drop
      return wrap(`
    ...
    // Chem fallback
    {
      ...
      while (labs.length < 5)
        labs.push(
          ["Reaction", "Structure", "Property", "Mechanism", "Equation"][
            labs.length
          ],
        );
  ...
  if (sub === "bio") {
    ...
    // Bio fallback
    {
      ...
      while (labs.length < 5)
        labs.push(
          ["Structure", "Function", "Process", "Regulation", "Adaptation"][
            labs.length
          ],
        );
  ...
  if (sub === "phy") {
    ...
    // Phy fallback
    {
      ...
      while (labs.length < 5)
        labs.push(
          ["Equation", "Principle", "Law", "Graph", "SI Units"][labs.length],
        );
      const sp = spoke(350, 148, 95, 5, -90);
      return wrap(`
        ${TT(topic.title)}
...
function resolveApiUrl(localPath) {
  ...
  if (localPath === "data/past-papers.json")
    return `${API_BASE_URL}/api/past-papers`;
  if (localPath === "data/community.json")
    return `${API_BASE_URL}/api/community`;
...
async function fetchJson(localPath, apiOverride = null) {
  const unwrap = (data) =>
    USE_BACKEND && data && data.success && data.data !== undefined
      ? data.data
      : data;
      ...
      try {
        ...
        if (localPath === "data/past-papers.json") {
          ...
          const allPlaceholder =
            papers.length > 0 &&
            papers.every((p) => !p.downloadUrl || p.downloadUrl === "#");
...
async function loadData() {
  try {
    console.log(
      `📚 Loading data [${USE_BACKEND ? "BACKEND: " + API_BASE_URL : "LOCAL FILES"}] …`,
    );
    ...
    try {
      ...
      if (localRes.ok) {
        ...
        const localSubjects = Array.isArray(localRaw)
          ? localRaw
          : localRaw?.subjects || [];
        state.subjects = mergeMissingSubjectTopics(
          state.subjects,
          localSubjects,
        );
      }
            ...
            fetchJson(path)
              ...
              .catch(() => {
                state.topics.set(topicRef.id, {
                  ...
                  concept: [
                    "Content file missing. Add this topic JSON in data/topics.",
                  ],
                  notes: [],
                  definitions: [],
                  workedExamples: [],
    ...
    if (
      state.community.forumThreads &&
      state.community.forumThreads.length > 0
    ) {
      state.selectedThreadId = state.community.forumThreads[0].id;
    }
    if (
      state.community.chatChannels &&
      state.community.chatChannels.length > 0
    ) {
    ...
    console.log(
      `✅ Data loaded successfully (${state.topics.size} topics, ${state.subjects.length} subjects)`,
    );
  } catch (error) {
...
function getISOWeek() {
  ...
  return (
    Math.floor((now - startOfWeek1) / (7 * 86400000)) +
    1 +
    "-" +
    now.getFullYear()
  );
  ...
  try {
    ...
    if (savedWeek !== thisWeek) {
      ...
      localStorage.setItem(
        weeklyMinutesKey,
        JSON.stringify(state.weeklyMinutes),
      );
      return;
    }
    ...
    if (raw) {
      ...
      if (Array.isArray(parsed) && parsed.length === 7) {
        state.weeklyMinutes = parsed.map((v) =>
          typeof v === "number" ? v : 0,
        );
      }
    }
...
async function saveProgressToBackend(topicId, quizScore, confidence) {
  ...
  try {
    const subject =
      state.topics.get(topicId)?.subject || state.currentSubject || "chem";
...
async function loadUserAnalytics() {
  ...
  try {
    const r = await fetch(`${API_BASE_URL}/api/user/analytics`, {
      headers: authHeaders(),
    });
...
async function getTopicProgressFromBackend(topicId) {
  if (!USE_BACKEND || !auth.isLoggedIn) return null;
  try {
    const r = await fetch(`${API_BASE_URL}/api/user/progress/${topicId}`, {
      headers: authHeaders(),
    });
    if (r.ok) return (await r.json()).data;
  } catch (e) {
    console.warn("Topic progress load failed:", e);
  }
  return null;
}
...
function computeSpacedRep() {
  ...
  quizHistory().forEach((q) => {
    if (q.topicId) quizScores[q.topicId] = q.scorePct;
  });
  const now = Date.now();
  const day = 86400000;
  const due = [];
  ...
  for (const subject of state.subjects) {
    for (const unit of subject.units) {
      for (const topic of unit.topics) {
        ...
        if (days >= interval) {
          let priority = 0;
          if (conf === "none" || conf === "no-idea")
            priority = 100 + Math.min(days, 50);
          else if (conf === "needs-practice")
            priority = 70 + Math.min(days, 30);
...
function renderHome() {
  ...
  const avgScore = avgQuiz.length
    ? Math.round(
        avgQuiz.reduce((sum, item) => sum + item.scorePct, 0) / avgQuiz.length,
      )
    : 0;
  ...
  if (subjProgressEl) {
    ...
    subjProgressEl.innerHTML = subjData
      .map(
        (s) => `
...
function renderSubjectSidebar(subject, activeTopicId = "") {
  ...
  const doneTopics = subject.units.reduce(
    (s, u) => s + u.topics.filter((t) => t.done).length,
    0,
  );
    ...
    <div id="sidebar-topics-list">
      ${subject.units
        .map((unit) => {
          ...
          const unitPct = unit.topics.length
            ? Math.round((unitDone / unit.topics.length) * 100)
            : 0;
          return `
...
function filterSidebarTopics(query) {
  ...
  document.querySelectorAll(".sidebar-group").forEach((group) => {
    const visible = [...group.querySelectorAll(".topic-item")].some(
...
async function ensureTopicLoaded(topicId) {
  ...
  const match = findTopicRefById(topicId);
  if (!match) {
    const dynamicSubjects = (state.subjects || [])
      .map((s) => s.id)
      .filter(Boolean);
    const candidates = [state.currentSubject, ...dynamicSubjects].filter(
      Boolean,
    );
  ...
  _sectionObserver = new IntersectionObserver(
    (entries) => {
      ...
      pills.forEach((btn) => {
        btn.classList.toggle(
          "active",
          btn.getAttribute("onclick")?.includes(topVisible),
        );
      });
    },
...
function renderTopicView(topicId) {
  ...
  const subject =
    state.subjectMap.get(topic.subject) ||
    state.subjectMap.get(state.currentSubject) ||
    state.subjects[0];
  if (!subject) {
    const main = byId("topic-main");
    if (main) {
  ...
  if (topicSidebar)
    topicSidebar.innerHTML = renderSubjectSidebar(subject, topic.id);
    ...
    <div class="topic-head card">
      ...
      <div class="topic-actions">
        ...
        <button class="btn btn-primary" onclick="App.go('quiz',{topicId:'${topic.id}'})">
          Topic Quiz${(() => {
            const scores = quizHistory().filter((q) => q.topicId === topic.id);
            const best = scores.length
              ? Math.max(...scores.map((q) => q.scorePct))
              : null;
            return best !== null
              ? ` <span class="quiz-best-chip">${best}%</span>`
              : "";
          })()}
        </button>
  ...
  if (topicMain && !topicMain.querySelector(".mobile-topics-toggle")) {
    ...
    togBtn.innerHTML =
      '<svg width="14" height="14" fill="none" viewBox="0 0 24 24"><path d="M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg> All Topics';
...
function revealNextStep(topicId, exampleIdx, total) {
  ...
  const counter = document.getElementById(
    `step-counter-${topicId}-${exampleIdx}`,
  );
...
function buildFlashDeck(payload) {
  ...
  for (const ref of refs) {
    ...
    cards.push(
      ...topic.flashcards.map((card) => ({ ...card, topic: topic.title })),
    );
  }
  return {
...
function renderFlashcard() {
  ...
  if (!flash || !flash.cards.length) {
    byId("flash-content").innerHTML =
      `<div class="card"><h2>No flashcards found</h2><p>Add flashcards arrays to topic JSON files.</p></div>`;
    return;
  }
...
async function submitAddPaper() {
  ...
  if (!variant) {
    if (status) {
      ...
      status.textContent = "Enter a variant (e.g. 11, 12)";
    }
    return;
  }
...
async function deletePaper(paperId, paperLabel) {
  ...
  try {
    const res = await fetch(
      API_BASE_URL + "/api/past-papers/" + encodeURIComponent(paperId),
      {
        method: "DELETE",
        headers: authHeaders(),
      },
...
function openPaperUrl(btnOrUrl, maybeUrl) {
  ...
  if (!rawUrl) {
    showToast("Paper URL is missing");
    return;
  }
...
function renderPastPapers() {
  ...
  if (
    savedFilters.subject &&
    ["all", "chem", "bio", "phy"].includes(savedFilters.subject)
  ) {
  ...
  if (tabBar) {
    ...
    if (!matched) {
      ...
      tabBar
        .querySelector('.papers-tab[data-subject="all"]')
  ...
  if (yearFilter && !yearFilter.dataset.ready) {
    ...
    const years = Array.from(new Set(state.pastPapers.map((p) => p.year))).sort(
      (a, b) => b - a,
    );
    ...
    byId("paper-session-filter")?.addEventListener(
      "change",
      _applyPaperFilters,
    );
    byId("paper-component-filter")?.addEventListener(
      "change",
      _applyPaperFilters,
    );
  }
  ...
  if (
    yearFilter &&
    savedFilters.year &&
    [...yearFilter.options].some((o) => o.value === savedFilters.year)
  ) {
    yearFilter.value = savedFilters.year;
  }
  if (
    sessionFilter &&
    savedFilters.session &&
    [...sessionFilter.options].some((o) => o.value === savedFilters.session)
  ) {
  ...
  if (
    ...
    [...componentFilter.options].some((o) => o.value === savedFilters.component)
  ) {
...
function _applyPaperFilters() {
  ...
  const papers = state.pastPapers.filter((p) => {
    if (_paperSubjectFilter !== "all" && p.subject !== _paperSubjectFilter)
      return false;
  ...
  if (!papers.length) {
    byId("past-paper-list").innerHTML =
      `<div class="paper-empty card"><p>No papers match your filters.</p></div>`;
    return;
  }
  ...
  for (const subj of subjectOrder) {
    ...
    for (const yr of years) {
      ...
      for (const paper of grouped[subj][yr]) {
        ...
        const diffClass =
          paper.difficulty === "High"
            ? "diff-high"
            : paper.difficulty === "Low"
              ? "diff-low"
              : "diff-med";
...
async function uploadAvatar(input) {
  if (!auth.isLoggedIn) {
    showToast("Sign in to upload an avatar");
    return;
  }
  const file = input.files[0];
  if (!file) return;
...
Current: ${current} min`,
  ...
  if (isNaN(val) || val < 10 || val > 1440) {
    showToast("Enter a number between 10 and 1440 minutes");
    return;
  }
...
function resetProgress() {
  if (
    !confirm(
      "This will erase ALL local progress, quiz history, confidence ratings, and study time.\n\nThis cannot be undone. Continue?",
    )
  )
    return;
...
function renderProfile() {
  ...
  if (user) {
    ...
    if (emailEl)
      emailEl.textContent = "Sign in to sync your progress across devices";
    byId("profile-xp").textContent = "0";
  }

  byId("profile-topics").textContent =
    overall.total > 0 ? `${overall.done}/${overall.total}` : "0";
  ...
  if (quizEl) {
    ...
    if (!history.length) {
      quizEl.innerHTML =
        '<p style="color:var(--text2);font-size:0.85rem;padding:0.5rem 0">No quizzes taken yet. Complete a topic quiz to see your history here.</p>';
    } else {
      const rows = history
        .map((h) => {
          const name =
            h.topicName || (h.topicId ? h.topicId.replace(/-/g, " ") : "Quiz");
          const date = new Date(h.at).toLocaleDateString(undefined, {
            day: "numeric",
            month: "short",
          });
          const cls =
...
function aiQuick(topicId, type) {
  if (!getAiEnabled()) {
    showToast("AI features are currently disabled.");
    return;
  }
  const prompts = {
  ...
  return arr
    ...
    .map((q, i) => {
      ...
      const answerLetter = String.fromCharCode(
        65 + Math.max(0, Math.min(3, Number(q.answerIndex || 0))),
      );
      return `Q${i + 1}. ${q.question || "Question"}\n${opts}\nAnswer: ${answerLetter}\nExplanation: ${q.explanation || ""}`;
    })
    .join("\n\n");
}

function formatAiLiteFlashcardText(payload) {
  const arr = Array.isArray(payload?.cards) ? payload.cards : [];
  if (!arr.length) return "No flashcards were generated.";
  return arr
    .slice(0, 8)
    .map(
      (c, i) =>
...
async function _runAiLite(topicId, mode) {
  if (!auth.isLoggedIn) {
    openAuthModal("login");
    return;
  }
  ...
  aiChatHistory[topicId].push({ role: "user", text: userLabel });
  renderAiChat(topicId);

  const loadingDiv = document.createElement("div");
  loadingDiv.className = "ai-bubble ai-bubble-assistant ai-bubble-loading";
  loadingDiv.innerHTML =
    '<span class="ai-dots"><span></span><span></span><span></span></span><span style="margin-left:0.5rem;font-size:0.82rem;color:var(--text3)">Preparing study content...</span>';
  ...
  try {
    ...
    if (!res.ok || !data.success) {
      aiChatHistory[topicId].push({
        role: "error",
        text: data.error || "Could not generate AI study content right now.",
      });
      renderAiChat(topicId);
      return;
    }
    ...
    const contentText =
      mode === "quiz"
        ? formatAiLiteQuizText(data.data)
        : formatAiLiteFlashcardText(data.data);

    const sourceLine = data.fallback
      ? "\n\nNote: Using fallback generator to protect free-tier limits."
      : "\n\nNote: Generated by AI-lite.";
    const quota = data.quota
      ? `\nDaily remaining: ${data.quota.dailyRemaining}/${data.quota.dailyMax} · Topic remaining: ${data.quota.topicRemaining}/${data.quota.topicMax}`
      : "";

    aiChatHistory[topicId].push({
      role: "assistant",
      text: `${contentText}${sourceLine}${quota}`,
    });
    renderAiChat(topicId);
  } catch {
    loadingDiv.remove();
    aiChatHistory[topicId].push({
      role: "error",
      text: "Network error while generating AI study content.",
    });
    renderAiChat(topicId);
  }
}
...
async function askAi(topicId) {
  ...
  loadingDiv.className = "ai-bubble ai-bubble-assistant ai-bubble-loading";
  loadingDiv.innerHTML =
    '<span class="ai-dots"><span></span><span></span><span></span></span><span style="margin-left:0.5rem;font-size:0.82rem;color:var(--text3)">🤔 Thinking...</span>';
  histEl.appendChild(loadingDiv);
  histEl.scrollTop = histEl.scrollHeight;

  if (sendBtn) {
    sendBtn.disabled = true;
    sendBtn.innerHTML = '<span style="opacity:0.6">⏳</span>';
  }
  if (promptEl) {
    promptEl.disabled = true;
    promptEl.placeholder = "Waiting for AI...";
  }
  ...
  if (promptEl) {
    ...
    promptEl.placeholder = `Ask anything about this topic…`;
    promptEl.focus();
  }
  renderAiChat(topicId);
}
...
function buildAiContext(topic) {
  if (!topic) return "";
  const parts = [];
  if (topic.concept?.length)
    parts.push("Concepts: " + topic.concept.slice(0, 2).join(" "));
  ...
  if (topic.tips?.length)
    parts.push("Exam tips: " + topic.tips.slice(0, 2).join("; "));
  return parts.join("\n").slice(0, 1200);
}
  ...
  const renderedBlocks = blocks
    .map((block) => {
      ...
      const linesHtml = nonEmpty.map((l) => {
        if (/^###\s+/.test(l))
          return `<h5>${inlineFormat(l.replace(/^###\s+/, ""))}</h5>`;
        if (/^##\s+/.test(l))
          return `<h4>${inlineFormat(l.replace(/^##\s+/, ""))}</h4>`;
        if (/^#\s+/.test(l))
          return `<h3>${inlineFormat(l.replace(/^#\s+/, ""))}</h3>`;
...
function renderAiChat(topicId) {
  ...
  if (!history.length) {
    histEl.innerHTML =
      '<p class="ai-empty">Ask anything about this topic — I\'ll help you understand, practise, or prepare for exams.</p>';
    return;
  }
...
function updateThemeIcon(theme) {
  ...
  if (theme === "dark") {
    ...
    icon.innerHTML =
      '<circle cx="12" cy="12" r="5" stroke="currentColor" stroke-width="2"/><path d="M12 1v2M12 21v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M1 12h2M21 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>';
  }
}
    ...
    if (discordCode && discordUri) {
      ...
      try {
        ...
        if (res.ok && data.token) {
          ...
          showToast(
            "Discord sign-in failed: " + (data.error || "Unknown error"),
          );
        }
      } catch {
        showToast("Discord sign-in failed — network error");
      }
    }
...
function doQuizImport() {
  ...
  if (!raw) {
    if (status) status.textContent = "Paste some JSON first.";
    return;
  }
  ...
  try {
    ...
    if (status) {
      status.className = "quiz-import-status error";
  ...
  if (!Array.isArray(parsed)) {
    ...
    if (parsed.questions && Array.isArray(parsed.questions)) {
      ...
      if (status) {
        status.className = "quiz-import-status error";
        status.textContent = "Expected a JSON array of questions.";
      }
      return;
    }
  }
  ...
  parsed.forEach((item, i) => {
    try {
      ...
      if (!Array.isArray(opts) || opts.length < 2) {
        errors.push("Item " + (i + 1) + ": need at least 2 options");
        return;
      }
      ...
      if (typeof ans === "string") {
        ...
        if (/^[A-D]$/.test(letter)) {
          ...
          const idx = opts.findIndex(
            (o) =>
              o.toString().toLowerCase().trim() === ans.toLowerCase().trim(),
          );
          ans = idx >= 0 ? idx : 0;
        }
      }
  ...
  if (!normalized.length) {
    if (status) {
      status.className = "quiz-import-status error";
      status.textContent = "No valid questions found. " + errors.join("; ");
    }
    return;
  }
  ...
  if (!topic) {
    if (status) {
      status.textContent = "Topic not found.";
    }
    return;
  }
  ...
  if (status) {
    status.className = "quiz-import-status success";
  ...
  showSuccess(
    ...
    normalized.length +
      " added to " +
      (topic.title || editorState.currentTopic),
  );
...
function _persistCustomTopic(topicId, data, subject) {
  try {
    ...
    console.warn("Could not persist custom topic:", e.message);
  }
}
...
function _restoreCustomTopics() {
  try {
      ...
      if (!state.topics.has(topicId)) {
        ...
        if (subj) {
          ...
          if (!found && subj.units.length > 0) {
            subj.units[0].topics.push({
              id: topicId,
              name: data.title || topicId,
              file: `${topicId}.json`,
            });
          }
        }
    ...
    if (restored > 0)
      console.log(`✅ Restored ${restored} custom topic(s) from localStorage`);
  } catch (e) {
    console.warn("Could not restore custom topics:", e.message);
  }
}
...
function filterEditorTopics(q) {
  ...
  items.forEach((item) => {
    item.style.display = item.textContent.toLowerCase().includes(ql)
      ? ""
      : "none";
  });
}
...
function _renderEditorForm(topic) {
  ...
  const notesText = (topic.notes || [])
    ...
    .join("\n\n");
  const defsText = (topic.definitions || [])
    .map((d) => `${d.term}: ${d.body}`)
    .join("\n");
  const recallText = (topic.recall || [])
    .map((r) => `${r.q}\n${r.a}`)
    .join("\n\n");
  const summaryText = (topic.summary || [])
    .map((s) => `${s.label || s.key || ""}: ${s.value || s.val || ""}`)
    .join("\n");
  const flashText = (topic.flashcards || [])
    .map((f) => `${f.q}\n${f.a}`)
    .join("\n\n");
...
function populateSubjectSelects(preferredSubjectId = "") {
  ...
  if (editorSelect) {
    ...
    editorSelect.innerHTML = `<option value="">Select subject…</option>${subjects
      ...
      .join("")}`;
    if (previous && subjects.some((s) => s.id === previous))
      editorSelect.value = previous;
  }
...
function renderSubjectSettingsPanel(subjectId) {
  ...
  if (!subject) {
    panel.innerHTML =
      "<h4>Subject Settings</h4><p>Select a subject to configure release status and metadata.</p>";
    return;
  }

  const color = normalizeHexColor(subject.primaryColor) || "#f97316";
  const sortOrder = Number.isFinite(Number(subject.sortOrder))
    ? Number(subject.sortOrder)
    : 999;
...
async function saveSubjectSettings() {
  ...
  if (state.currentView === "subject" && state.currentSubject === optimistic.id)
    renderSubjectView(optimistic.id);
  if (
    state.currentView === "topic" &&
    state.currentSubject === optimistic.id &&
    state.currentTopic
  )
    renderTopicView(state.currentTopic);
  ...
  try {
    ...
    if (state.currentView === "subject" && state.currentSubject === updated.id)
      renderSubjectView(updated.id);
    if (
      state.currentView === "topic" &&
      state.currentSubject === updated.id &&
      state.currentTopic
    )
      renderTopicView(state.currentTopic);
    if (state.currentView === "subjects") renderSubjectSelection();
...
function openTopicInEditor(topicId) {
  ...
  const _qib = byId("quiz-import-btn");
  if (_qib) _qib.style.display = "";
...
function createNewTopic() {
  ...
  const newTopic = {
    ...
    workedExamples: [
      {
        q: "Example question?",
        steps: [{ n: 1, sub: "Identify", text: "What is given?" }],
      },
    ],
...
function openAddSubjectModal() {
  ...
  if (nameEl && idEl) {
    nameEl.addEventListener("input", () => {
      if (!idEl.value.trim())
        idEl.value = slugifyTopicId(nameEl.value).slice(0, 16);
    });
  }
  modal.addEventListener("click", (e) => {
    if (e.target === modal) modal.remove();
...
function showEditorHelp() {
  showToast(
    "Editor Help: Edit JSON to modify topic properties. Save sends to backend. Delete removes from system.",
  );
}
...
async function renderChatSidebar() {
  ...
  if (chList)
    chList.innerHTML = state.community.chatChannels
      .map(
        (ch) => `
  ...
  if (!container) return;
  container.innerHTML =
    '<p style="color:var(--text2);padding:0.5rem;font-size:0.8rem">Loading messages…</p>';
...
async function renderCommunity(forceRefresh = false) {
  ...
  if (forceRefresh || now - _communityLastFetch > 30000) {
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/community/forum?sort=pinned&limit=50`,
      );
      const data = await res.json();
      if (data.success) {
        state.community.forumThreads = data.data;
        _communityLastFetch = now;
      }
    } catch {
      /* use cached */
    }
  ...
  // Build thread list
  const forumItemsHtml = visibleThreads
    .map((thread) => {
      ...
      const pinnedBadge = thread.pinned
        ? '<span class="thread-badge pinned">📌 Pinned</span>'
        : "";
      const lockedBadge = thread.locked
        ? '<span class="thread-badge locked">🔒 Locked</span>'
        : "";
      return `
...
async function renderThreadDetail() {
  ...
  const repliesHtml = replies
    .map((r) => {
      ...
      const canDel =
        auth.isLoggedIn && (isAdmin || auth.user?.name === r.author);
      return `
  ...
  const deleteBtn =
    isAdmin || isAuthor
      ? `<button class="btn btn-outline btn-sm btn-danger" onclick="App.deleteThread('${id}')">Delete Thread</button>`
      : "";

  const replyBox =
    auth.isLoggedIn && !thread.locked
      ? `
  ...
  `
      : auth.isLoggedIn && thread.locked
        ? `<p style="color:var(--text2);font-size:0.85rem;margin-top:0.75rem">🔒 This thread is locked.</p>`
...
async function selectThread(threadId) {
  ...
  document.querySelectorAll(".forum-item").forEach((btn) => {
    btn.classList.toggle(
      "active",
      btn.getAttribute("onclick")?.includes(threadId),
    );
  });
}
...
async function submitReply(threadId) {
  ...
  try {
    const res = await fetch(
      `${API_BASE_URL}/api/community/forum/${threadId}/replies`,
      {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({ body }),
    ...
    if (updatedThread && updatedThread._id) {
      const idx = state.community.forumThreads.findIndex(
        (t) => (t._id || t.id) === threadId,
      );
    ...
    document.querySelectorAll(".forum-item").forEach((btn) => {
      ...
      if (onclick.includes(threadId)) {
        const countSpan = [...btn.querySelectorAll(".forum-meta span")].find(
          (s) => s.textContent.includes("repl"),
        );
        if (countSpan && updatedThread)
          countSpan.textContent = `${updatedThread.replies?.length || 0} replies`;
      }
    });
  } catch {
    showToast("Network error");
...
async function deleteReply(threadId, replyId) {
  ...
  try {
    const res = await fetch(
      `${API_BASE_URL}/api/community/forum/${threadId}/replies/${replyId}`,
      {
        method: "DELETE",
        headers: authHeaders(),
      },
...
async function deleteThread(threadId) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      state.community.forumThreads = state.community.forumThreads.filter(
        (t) => (t._id || t.id) !== threadId,
      );
...
async function upvoteThread(threadId) {
  ...
  try {
    const res = await fetch(
      `${API_BASE_URL}/api/community/forum/${threadId}/upvote`,
      {
        method: "POST",
        headers: authHeaders(),
      },
...
function openNewThreadModal() {
  ...
  if (!modal) {
    ...
    document.body.appendChild(modal);
    modal.addEventListener("click", (e) => {
      if (e.target === modal) closeNewThreadModal();
    });
  }
...
async function submitNewThread() {
  ...
  if (!title || !body) {
    if (errEl) errEl.textContent = "Title and body are required.";
    return;
  }
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) {
      if (errEl) errEl.textContent = data.error || "Failed to post.";
      return;
    }
...
function switchAdminTab(tab) {
  ...
  document
    ...
    .forEach((p) => p.classList.toggle("active", p.id === `admin-tab-${tab}`));
    ...
    if (ru)
      ...
      <div class="admin-recent-item">
        ...
        <small>${new Date(u.createdAt).toLocaleDateString()}</small>
          ...
          .join("") ||
        '<p style="color:var(--text2);font-size:0.85rem">No users yet.</p>';
    ...
    if (rt)
      ...
      <div class="admin-recent-item">
        ...
        <small>${new Date(t.createdAt).toLocaleDateString()}</small>
          ...
          .join("") ||
        '<p style="color:var(--text2);font-size:0.85rem">No threads yet.</p>';
...
async function loadAdminOpenRouterModels() {
  ...
  if (!card) return;
  card.innerHTML =
    '<p style="color:var(--text2);font-size:0.85rem">Loading AI model diagnostics…</p>';
  try {
    const res = await fetch(`${API_BASE_URL}/api/admin/openrouter-models`, {
      headers: authHeaders(),
    });
    const data = await res.json();
    if (!res.ok || !data.success) {
...
async function loadAdminUsers() {
  ...
  if (!tbody) return;
  tbody.innerHTML =
    '<tr><td colspan="5" style="color:var(--text2);text-align:center;padding:1rem">Loading…</td></tr>';
  try {
    ...
    tbody.innerHTML = `<tr><td colspan="5" style="color:#ef4444;padding:1rem">Network error: ${escapeHtml(e.message)}</td></tr>`;
  }
}
...
function renderAdminUsersTable(users) {
  ...
  if (!users.length) {
    tbody.innerHTML =
      '<tr><td colspan="5" style="color:var(--text2);text-align:center;padding:1rem">No users found.</td></tr>';
    return;
  }
  tbody.innerHTML = users
    .map((u) => {
      ...
      const roleBadge =
        ...
        "</span>" +
        (isBanned
          ? ' <span class="role-badge role-student">banned</span>'
          : "");
      const actions = isSelf
        ? '<em style="color:var(--text2);font-size:0.8rem">You</em>'
        : `
...
async function toggleUserRole(userId, currentRole) {
  ...
  const labels = {
    student: "Student",
    teacher: "Teacher (can edit topics)",
  ...
  if (!newRole || !options.includes(newRole.trim().toLowerCase())) {
    if (newRole !== null)
      showToast("Invalid role — use: student, teacher, or admin");
    return;
  }
...
async function loadAdminForum() {
  ...
  if (!tbody) return;
  tbody.innerHTML =
    '<tr><td colspan="6" style="color:var(--text2);text-align:center;padding:1rem">Loading…</td></tr>';
  try {
    const res = await fetch(`${API_BASE_URL}/api/admin/forum`, {
      headers: authHeaders(),
    });
    const data = await res.json();
    if (!res.ok || !data.success) {
    ...
    tbody.innerHTML = `<tr><td colspan="6" style="color:#ef4444;padding:1rem">Network error: ${escapeHtml(e.message)}</td></tr>`;
  }
}
...
function renderAdminForumTable(threads) {
  ...
  if (!threads.length) {
    tbody.innerHTML =
      '<tr><td colspan="6" style="color:var(--text2);text-align:center;padding:1rem">No threads.</td></tr>';
    return;
  }
    ...
    .map((t) => {
      ...
      const status =
        [
          ...
          .filter(Boolean)
          .join(" ") ||
        '<span style="color:var(--text2);font-size:0.8rem">Normal</span>';
      return `<tr>
...
async function adminLockThread(threadId, locked) {
  try {
    const res = await fetch(
      `${API_BASE_URL}/api/admin/forum/${threadId}/lock`,
      {
        method: "PATCH",
        headers: authHeaders(),
...
function loadAdminTopicList(subjectId) {
  ...
  if (!subject) {
    container.innerHTML =
      '<p style="color:var(--text2)">Subject not found.</p>';
    return;
  }
  const allTopics = subject.units.flatMap((u) =>
    u.topics.map((t) => ({ ...t, unitName: u.name })),
  );
  if (!allTopics.length) {
    container.innerHTML =
...
async function saveTopic() {
  ...
  try {
    ...
    const normalized = {
      ...
      title:
        parsed.title ||
        state.topics.get(editorState.currentTopic)?.title ||
        editorState.currentTopic,
    };
    ...
    if (
      ...
      const putRes = await fetch(
        `${API_BASE_URL}/api/topics/${editorState.currentTopic}`,
        {
          method: "PUT",
          headers: authHeaders(),
          body: JSON.stringify({
      ...
      if (!putRes.ok && putRes.status === 404) {
        ...
        if (!postRes.ok) {
          showToast(
            `Save error: ${postData.error || "Could not create topic"}`,
          );
          return;
        }
      } else if (!putRes.ok) {
    ...
    if (
      state.currentView === "topic" &&
      state.currentTopic === editorState.currentTopic
    ) {
...
async function deleteCurrentTopic() {
  if (
    !editorState.currentTopic ||
    !confirm("Really delete this topic? This cannot be undone.")
  )
    return;
...
function getActiveSidebarElement() {
  return (
    document.querySelector(".view.active .sidebar") ||
...
function closeMobileSidebar() {
  document
    .querySelectorAll(".sidebar.mobile-open")
    .forEach((sidebar) => sidebar.classList.remove("mobile-open"));
  const backdrop = document.getElementById("sidebar-backdrop");
...
function go(viewName, payload = {}) {
  ...
  if (
    viewName === "subject" &&
    targetSubjectId &&
    !canAccessSubject(targetSubjectId)
  ) {
  ...
  if (
    (viewName === "topic" || viewName === "quiz" || viewName === "flash") &&
    targetTopicId &&
    !canAccessTopic(targetTopicId)
  ) {
  ...
  if (
    ...
    (viewName === "topic" || viewName === "subject")
  ) {
    closeMobileSidebar();
  }
  ...
  if (viewName === "subject")
    renderSubjectView(payload.subjectId || state.currentSubject);
  if (viewName === "topic")
    renderTopicView(payload.topicId || state.currentTopic);
  ...
  if (viewName === "editor") {
    ...
    if (ss)
      ss.innerHTML =
        "<h4>Subject Settings</h4><p>Select a subject to configure release status, colours, and metadata.</p>";
    const duplicateBtn = byId("editor-duplicate-btn");
    if (duplicateBtn) duplicateBtn.style.display = "none";
  }
  ...
  if (mbnId) {
    const btn = document.getElementById(mbnId);
...
function bindBaseEvents() {
  ...
  if (hamburger) {
    ...
    document.addEventListener("click", (e) => {
      if (
        ...
        byId("nav-links")?.classList.contains("open")
      ) {
  ...
  document.addEventListener("input", (e) => {
    if (
      ...
      e.target.dataset.autoresize !== undefined
    ) {
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (e.key === "Escape") {
      ...
      if (ntModal?.classList.contains("open")) {
        closeNewThreadModal();
        return;
      }
    ...
    if (!inInput && e.key === "?" && e.shiftKey) {
      showToast(
        "⌨  /=search  J/K=navigate  Q=quiz  F=flashcards  P=print  Esc=close",
      );
      return;
    }
    ...
    if (!inInput && state.currentView === "topic") {
      if (e.key === "j" || e.key === "k") {
        ...
        if (next) {
          e.preventDefault();
          go("topic", { topicId: next.id });
        }
        return;
      }
      if (e.key === "q") {
        e.preventDefault();
        go("quiz", { topicId: state.currentTopic });
        return;
      }
      if (e.key === "f") {
        e.preventDefault();
        go("flash", { topicId: state.currentTopic });
        return;
      }
...
function tpGenerate() {
  ...
  const typeLabel = {
    mcq: "Paper 1 — Multiple Choice",
    structured: "Paper 2 — Structured Questions",
...
function tpRevealAnswer() {
  ...
  if (reveal) {
    ...
    reveal.classList.add("tp-reveal-in");
  }
...
function tpSelectOpt(idx, correct) {
  ...
  document.querySelectorAll(".tp-opt").forEach((el, i) => {
    if (i === idx)
      el.classList.add(idx === correct ? "tp-opt-correct" : "tp-opt-wrong");
...
function switchSocialTab(tab, btn) {
  document
    .querySelectorAll(".social-nav-tab")
    .forEach((b) => b.classList.remove("active"));
  if (btn) btn.classList.add("active");
  document
...
async function _initGroupsTab() {
  if (!auth.isLoggedIn) {
    ...
    if (main)
      main.innerHTML = `<div class="social-empty-state" style="flex:1;justify-content:center"><div style="font-size:2rem">🗨</div><p>Sign in to use group chats</p><button class="btn btn-primary btn-sm" onclick="App.openAuthModal('login')">Sign In</button></div>`;
    return;
  }
  try {
    ...
    console.warn("Groups load:", e.message);
  }
  _renderGroupList();
}
...
function _renderGroupList() {
  ...
  el.innerHTML = _socialState.groups
    .map(
      (g) => `
...
async function socialSearch(query) {
  ...
  resultsEl.innerHTML =
    '<div class="social-search-empty" style="color:var(--text3);padding:0.5rem 0.75rem">Searching…</div>';
  ...
  _socialSearchTimer = setTimeout(async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/social/search?q=${encodeURIComponent(q)}`,
        { headers: authHeaders() },
      );
      const data = await res.json();
      ...
      if (!results.length) {
        ...
        el.innerHTML = results
          .map((u) => {
            const alreadyFriend = _socialState.friends.some(
      ...
      if (el2)
        el2.innerHTML =
          '<div class="social-search-empty">Search failed — check connection</div>';
...
document.addEventListener("click", (e) => {
  ...
  const bar = input?.closest(
    ".social-search-wrap, .social-search-bar, .social-search-field",
  );
  if (!wrap) return;
  // Only close if the click was outside both the input and the results
  const clickedInsideSearch =
    (bar && bar.contains(e.target)) || wrap.contains(e.target);
...
async function openUserProfile(userId) {
  try {
    const res = await fetch(`${API_BASE_URL}/api/social/profile/${userId}`, {
      headers: authHeaders(),
    });
    const data = await res.json();
    if (!data.success) {
...
async function openGroupChat(groupId, groupName) {
  ...
  try {
    const res = await fetch(
      `${API_BASE_URL}/api/social/groups/${groupId}/messages`,
      { headers: authHeaders() },
    );
    const data = await res.json();
    ...
    if (msgsEl)
      msgsEl.innerHTML =
        '<div class="social-msgs-error">Could not load messages — check connection.</div>';
...
async function sendGroupMessage() {
  ...
  try {
    const res = await fetch(
      `${API_BASE_URL}/api/social/groups/${_socialState.activeGroup._id}/messages`,
      {
        method: "POST",
        headers: authHeaders(),
...
function _renderFriendPanel() {
  ...
  const pending = _socialState.requests.filter(
    (r) => r.to?.toString?.() === auth.user?._id || r.to === auth.user?._id,
  );
  ...
  const friendsHtml =
    ...
    <div class="social-user-row" onclick="App.openUserProfile('${f._id}')">
      ...
      .join("") ||
    '<p class="social-empty">No friends yet — search for users to connect!</p>';
...
function _renderGroupPanel() {
  ...
  const groupsHtml =
    ...
    <button class="social-group-item ${_socialState.activeGroup?._id === g._id ? "active" : ""}"
      ...
      .join("") || '<p class="social-empty">No group chats yet.</p>';
...
function _groupMsgHtml(m) {
  const isSelf =
    m.authorId === auth.user?._id ||
    m.authorId?.toString?.() === auth.user?._id;
...
function showSuccess(msg, sub) {
  ...
  el.innerHTML =
    '<div class="success-popup-inner"><span class="success-popup-icon">&#x2705;</span>' +
    '<div class="success-popup-text"><strong>' +
    escapeHtml(msg) +
    "</strong>" +
    subHtml +
    "</div>" +
